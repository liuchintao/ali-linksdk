...
/*********************************************************
 * 接口名称：get_ra_buffer_write_pointer
 * 描       述：获取Buffer的写地址
 * 输入参数：RA_BUFFER_INFO_S* buffer
 * 输出参数：
 * 返  回 值：
...
/*********************************************************
 * 接口名称：move_ra_buffer_write_pointer
 * 描       述：移动buffer读指针
 * 输入参数：RA_BUFFER_INFO_S* buffer
 * 输出参数：
 * 返  回 值：
...
{
    int len = get_ra_buffer_read_len(buffer);
    if(offset_len > 0)  //右移
    {
        if(buffer->write_index + offset_len > buffer->size)
        {
...
/*********************************************************
 * 接口名称：reset_ra_buffer_write_point
 * 描       述：重置RABuffer的写索引
 * 输入参数：RA_BUFFER_INFO_S* buffer
 * 输出参数：
 * 返  回 值：
...
/*********************************************************
 * 接口名称：join_content_before_ra_buffer
...
{
    int remain_len = get_ra_buffer_read_len(channel_buffer);

    if( 0 != memmove_ra_buffer(channel_buffer,len))
    {
        return -1;
    }

    reset_ra_buffer_read_point(channel_buffer);
    reset_ra_buffer_write_point(channel_buffer);

    if(0 != write_ra_buffer(channel_buffer,data,len))
    {
        return -1;
    }

    if(0 != move_ra_buffer_write_pointer(channel_buffer,remain_len))
    {
        return -1;
    }
